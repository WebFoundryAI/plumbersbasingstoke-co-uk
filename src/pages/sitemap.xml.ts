...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Basingstoke gets higher priority)
    const isBasingstoke = location.slug === 'basingstoke';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isBasingstoke ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isBasingstoke ? 1 : 2 + i),
    });
    ...
    for (const [j, service] of SERVICES.entries()) {
      urls.push({
        ...
        priority: isBasingstoke ? 0.8 : 0.7,
        changefreq: 'monthly',
